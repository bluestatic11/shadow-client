...
# (slug, description, critical?) â€” ordered by impact on FPS.
#
# The list includes a few slugs that don't yet ship a 1.21.11 build
# (nvidium, ebe) â€” _pick_version skips them with a "skip" message and
...
PERFORMANCE_MODS: list[tuple[str, str, bool]] = [
    ...
    ("sodium",             "modern rendering engine â€” biggest FPS win", True),
    ...
    ("ebe",                "Enhanced Block Entities â€” baked chests",    False),
    ...
    ("threadtweak",        "thread scheduling â€” smoother frametimes",   False),
    ...
    # --- entity / world tuning -------------------------------------------
    ("lmd",                "Let Me Despawn â€” fewer idle mobs",          False),
    ...
    ("distanthorizons",    "LOD renderer â€” huge render distance for cheap", False),
    ...
    # Only rendering/info/QoL â€” nothing that changes reach, targeting, or
    ...
    # the crash-free days) and modifies fire-OVERLAY rendering â€” a render-pass
    ...
    ("dynamic-fire-overlay","low fire â€” shrink/lower the burning overlay", False),
    ("shieldscale",        "low shield â€” custom shield scale/position", False),
    ...
    # round-trip. Allowed on most CPvP servers, but rules vary â€” some
    # (e.g. VTL) ban optimizers outright; that's on the player to check.
    ("marlow-crystal-optimizer", "crystal PvP â€” no wait on server crystal cleanup", False),
    ...
    ("anchor-optimizer",   "anchor PvP â€” no wait on server anchor cleanup", False),
    ("no-death-animation", "instant death poof â€” see totem pops clearly", False),
    # 2.9M dl. Overlaps ShadowHud's NoHurtCam module â€” leave that module OFF
    ...
    # slug really is "cpvp" â€” this is Totem Tweaks (2M dl): small totem pop.
    ...
    ("cpvp",               "Totem Tweaks â€” small totem pop, custom pop anim", False),
    ("capes",              "cape support (OptiFine/Cosmetica capes render)", False),
    # (re-added 2026-07-21 â€” cleared of suspicion; crashes traced to
    ...
    ("shulkerboxtooltip", "hover a shulker box to see its contents",  False),
    # clears the fog/overlay when your camera is submerged in water/lava/
    # powder snow â€” see through it clearly. Rendering-only (honor-system on
    ...
    # "Pose stack not empty" PvP crashes â€” if they recur, first try setting
    ...
    # These aren't FPS mods themselves â€” they're libraries other mods pull
...
# newest build. Iris and Sodium ship as a matched pair â€” each Iris release
...
# that supports a newer Sodium, bump or delete this entry** â€” check the Iris
...
VERSION_PINS: dict[str, str] = {
    ...
    # Sodium Extra at 0.9.1 (last build that works with 0.8.12 â€” verified
...
def _pick_version(slug: str, mc_version: str, loader: str = "fabric") -> dict[str, Any] | None:
    ...
    had changed â€” see ModernFix/Noisium earlier. Better to miss a mod than
...
# â”€â”€â”€ Direct-URL mods (not on Modrinth) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# launcher's MOD_JAR_URL in launcher/src-tauri/src/shadow_chat.rs â€” both
...
def _install_shadow_chat(mods_dir: Path, mc_version: str) -> bool:
    ...
    shadow-chat-*.jar files â€” if the release isn't published yet (launcher
    ...
    if mc_version not in SHADOW_CHAT_MC_VERSIONS:
        print(f"[mods]   skip shadow-chat â€” not built for MC {mc_version}")
        return False
    ...
    try:
        ...
        print(f"[mods]   shadow-chat download failed ({e}) â€” keeping any existing jar")
        return False
    for stale in mods_dir.glob("shadow-chat-*.jar"):
        if stale.name != filename:
            try:
                ...
                # Locked (MC running) â€” Fabric would see a duplicate next
                ...
                try:
                    ...
                    print(f"[mods]   {stale.name} is locked â€” staged {filename}.new for next launch")
                except OSError:
                    pass
                return False
    return True
...
def install_mods(mods_dir: Path, mc_version: str) -> tuple[list[str], list[str]]:
    ...
    for slug, desc, critical in PERFORMANCE_MODS:
        ...
        if not v:
            msg = f"  skip {slug} â€” no {mc_version} fabric build"
            if critical:
                msg += "  (!)"
            print(f"[mods] {msg}")
            skipped.append(slug)
        ...
        print(f"[mods] {slug:18} â†’ {file['filename']}  ({desc})")
